  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.azerothcore-world = {
      ...
      serviceConfig = {
        ...
        # страницы worldserver в swap (zram→диск), НЕ убивая процесс. Сглаживает
        # пик разворота 1000 ботов, из-за которого 2 авг случился livelock.
        # MemoryMax — ЖЁСТКИЙ backstop: гарантирует ~6 ГБ RAM остальной системе,
        # чтобы worldserver физически не мог повесить Hyprland. Достигается редко;
        # при достижении процесс будет OOM-killed внутри cgroup (сервер упадёт и
        # перезапустится по Restart=on-failure) — поэтому потолок высокий.
        # Значения под 32 ГБ RAM; при желании тюнить здесь.
        MemoryHigh    = "18G";
        MemoryMax     = "23G";
        MemorySwapMax = "12G"; # спилл до 12G, ~11G свопа остаётся остальной системе
        # Не давать systemd-oomd убивать сервер по давлению — им управляем вручную
        # через MemoryHigh/Max выше; oomd защищает остальную систему.
